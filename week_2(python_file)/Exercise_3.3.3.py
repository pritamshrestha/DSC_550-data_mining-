...
# FileName:Exercise_3.3.3
# Date:6/05/2020
# Course: DSC550-Data Mining
# Professor/Instructor:Brant Abeln
# Description:Uses of minhas signatures and jaccard similarities
# Due Date:06/20/20
# Assignment No:2.1
...
# Question:
# (a) Compute the minhash signature for each columns if we use the following three hash functions:h1(x)=2X+1 mod6;
# h2x=3x+2 mod 6; h3(x)=5x+2 mod 6.
...
def create_combined_matrix(hash_list_1, hash_list_2, hash_list_3):
    ...
    for i in range(0, len(element_list)):
        hash_matrix.loc[i] = [element_list[i], set1[i], set2[i], set3[i], set4[i], hash_list_1[i], hash_list_2[i], hash_list_3[i]]
    # list full matrix
...
def main():
    ...
    try:
        ...
        print('\nColumn 1 and Column 2: ' + str(intersection_cardinality(set1, set2)) + ' / ' + str(union_cardinality(set1,set2)) + str(' or ' + str(jaccard_similarity(set1, set2))))
        print('Column 1 and Column 3: ' + str(intersection_cardinality(set1, set3)) + ' / ' + str(union_cardinality(set1,set3)) + str(' or ' + str(jaccard_similarity(set1, set3))))
        print('Column 1 and Column 4: ' + str(intersection_cardinality(set1, set4)) + ' / ' + str(union_cardinality(set1,set4)) + str(' or ' + str(jaccard_similarity(set1, set4))))
        print('Column 2 and Column 3: ' + str(intersection_cardinality(set2, set3)) + ' / ' + str(union_cardinality(set2,set3)) + str(' or ' + str(jaccard_similarity(set2, set3))))
        print('Column 2 and Column 4: ' + str(intersection_cardinality(set2, set4)) + ' / ' + str(union_cardinality(set2,set4)) + str(' or ' + str(jaccard_similarity(set2, set4)))) 
        print('Column 3 and Column 4: ' + str(intersection_cardinality(set3, set4)) + ' / ' + str(union_cardinality(set3,set4)) + str(' or ' + str(jaccard_similarity(set3, set4))))
